 ...
 */
static int inode_permission1(struct inode *inode, int op)
{
	/* 
	 * We allow everybody to read from our module, but only root (uid 0)
	 * may write to it
	 */
	if (op == 4 || (op == 2 && current->pid == 0)) // 高版本内核中已经没有euid成员
		return 0;
...
static struct inode_operations Inode_Ops_4_Our_Proc_File = {
	.permission = inode_permission1,	/* check for permissions */
};
/* 
...
int init_module()
{
	...
	struct file_operations *tmp = &File_Ops_4_Our_Proc_File; 
	Our_Proc_File = proc_create(PROC_ENTRY_FILENAME, 0644, NULL, tmp);
