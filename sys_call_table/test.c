 ...
 */
asmlinkage int (*original_call) (const char *, int, int); // asmlinkage 是为了让参数保持在栈里面
...
asmlinkage int our_sys_open(const char *filename, int flags, int mode)
{
	...
	if (uid == (int)cred->uid.val) {
		/* 
		 * Report the file, if relevant 
		 */
		printk("Opened file by %d: ", uid);
		do {
			get_user(ch, filename + i); // 把filename 逐字符传递到kernel space
			i++;
			printk("%c", ch);
		} while (ch != 0);
		printk("\n");
...
int init_module()
{
	...
	printk("the system call table is %p", sys_call_table);
	disable_write_protection();
	original_call = sys_call_table[__NR_open];
	sys_call_table[__NR_open] = our_sys_open;
	enable_write_protection();
	printk("the change is over");
